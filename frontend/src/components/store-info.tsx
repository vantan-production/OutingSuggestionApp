"use client";
import { DEFAULT_LAT, DEFAULT_LON, Place, calculateDistance, storeInfo as fetchStoreInfo } from "@/api/places";
import { Contrail_One } from "next/font/google";
import { useEffect, useState } from "react";

export function StoreInfo () {

    const [storeInfo, setStoreInfo] = useState<Place[]>([]);
    const [error, setError] = useState<string>("");
    const [isLoading, setIsLoading] = useState<boolean>(false);
    const [currentLocation, setCurrentLocation] = useState<{lat: number, lon: number}>({lat: DEFAULT_LAT, lon: DEFAULT_LON});
    ...
    // 距離を計算する関数
    const getDistance = (store: Place | undefined): string => {
    ...
    const getBusiness = (store: Place | undefined): string => {
        if(!store) {
            return "-";
        }
...
const getReview = (store: Place | undefined): string => {
    if (!store) {
        return "-";
    }
    if (store.reviews.length > 0 && store.reviews.length < 1) {
        return "1つ星";
    } else if (store.reviews.length > 1 && store.reviews.length < 2) {
        return "2つ星";
    } else if (store.reviews.length > 2 && store.reviews.length < 3) {
        return "3つ星";
    } else if (store.reviews.length > 3 && store.reviews.length < 4) {
        return "4つ星";
    } else if (store.reviews.length > 4 && store.reviews.length < 5) {
        return "5つ星";
    } else {    
        return (
            <p className="small text-black">評価がありません</p>
        );
    }
}
