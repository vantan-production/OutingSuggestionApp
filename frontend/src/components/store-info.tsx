...
export function StoreInfo() {
  const [storeInfo, setStoreInfo] = useState<PlaceResponse["places"]>([]);
  ...
  useEffect(() => {
    ...
    place({
      ...
      .then((res) => {
        console.log(res);
        if (res) {
          const places = res.places.map((place) => {
            return {
              id: place.id,
              name: place.name,
  ...
  return (
    <div className=" w-44 rounded-lg overflow-hidden">
      <div>
        {storeInfo[0]?.photos[0]?.url && (
          <img
            className=" w-44 h-24 "
            src={storeInfo[0]?.photos[0]?.url ?? ""}
            alt="store-photo"
          />
        )}
      </div>
        ...
        <div className=" w-40 flex mx-1 mb-1">
          <div className=" flex ml-5">
            ...
            <p className="small ml-1 h-2 w-full">
              {storeInfo[0]?.reviews[0]?.text.text}
            </p>
          </div>
        </div>
      </div>
